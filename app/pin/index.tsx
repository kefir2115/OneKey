...
export default function Pin() {
    ...
    useEffect(() => {
        if (config.biometric && config.pin.length > 0 && route.path?.includes("pin")) {
            bioAuth();
        }
    }, []);
    ...
    useEffect(() => {
        if (nums.length === 4 && prevPin.length === 4) {
            ...
            if (!isCorrect) {
                ...
                Crypto.digestStringAsync(Crypto.CryptoDigestAlgorithm.SHA256, nums.join(",")).then((sha) => {
                    ...
                    router.replace((params.next as any) || "/base");
                });
            }
        }
        if (nums.length === 4 && prevPin.length === 0) {
            if (config.pin.length === 0) {
                ...
                Crypto.digestStringAsync(Crypto.CryptoDigestAlgorithm.SHA256, nums.join(",")).then((sha) => {
                    if (config.pin === sha) router.replace((params.next as any) || "/base");
                    else {
                        setNums([]);
                        showDialog();
                        setTries((t) => --t);
    ...
    const bioAuth = () => {
        authenticateAsync().then((r) => {
            if (r.success) {
                router.navigate((params.next as any) || "/base");
            }
        });
    };
